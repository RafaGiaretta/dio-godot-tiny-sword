...
# Obter input_vector
	...
	var deadzone = 0.25
	if abs(input_vector.x) < 0.25:
		input_vector.x = 0.0
	if abs(input_vector.y) < 0.25:
		input_vector.y = 0.0
	
	# Atualizar is_running
	was_running = is_running
	is_running = not input_vector.is_zero_approx()
func play_run_idle_animation()-> void :	if not is_attacking:
...
func update_hibox_detection(delta: float) -> void:
	# Temporizador
...
func damage(amount: int) -> void:
	...
	health -= amount
	print("Player dano:", amount,  " vida total: ", health)
